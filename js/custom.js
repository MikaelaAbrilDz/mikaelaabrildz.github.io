...
$('.popup-image').magnificPopup({
		...
		callbacks: {
			open: function popup() {`
		// Agregar contenido personalizado al popup al abrirlo
		var imageSrc = this.st.items[0].src; // Obtiene la fuente de la imagen
		var title = this.st.items[0].title || "Título del Proyecto"; // Título opcional

		// Generamos el HTML del contenido para el popup
		var customContent =
                <div class="popup-content">
                    <div class="popup-image-wrapper">
                        <img src="${imageSrc}" alt="Imagen de trabajo" class="popup-image">
                        <div class="popup-text">
                            <h2>${title}</h2>
                            <p>Descripción del proyecto o detalles del trabajo mostrado. Puedes agregar más texto aquí según sea necesario.</p>
                        </div>
                    </div>
                </div>
            `;

		// Inyectamos el HTML personalizado en el contenedor del popup
		this.contentContainer.html(customContent);
	}
};
$('.open-custom-popup').magnificPopup({
	type: 'inline',
	midClick: true, // permite abrir en clics medios
	mainClass: 'mfp-fade'
});
